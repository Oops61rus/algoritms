...
function checkPhone(savedPhoneNumber, newPhoneOne, newPhoneTwo, newPhoneThree) {
  const defaultCode = 495;
  const regex = /\D/g;
  const oldFixedPhone = defaultCode + savedPhoneNumber.replace(regex, '');

  const list = [
    defaultCode + newPhoneOne.replace(regex, ''),
    defaultCode + newPhoneTwo.replace(regex, ''),
    defaultCode + newPhoneThree.replace(regex, '')
  ]

  return list.map(phone => oldFixedPhone.slice(-10) === phone.slice(-10) ? 'YES' : 'NO').join('\n')
}
