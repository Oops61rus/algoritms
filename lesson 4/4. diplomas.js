...
const [w, h, n] = fileContent.toString().trim().split(' ');

function diplomas(w, h, n) {
  let left = 0;
  let right = Math.max(w, h) * n;

  while (right - left > 0) {
    let mid = Math.floor((left + right) / 2)

    const floorW = Math.floor(mid / w)
    const floorH = Math.floor(mid / h)
...
const result = diplomas(Number(w), Number(h), Number(n));

fs.writeFileSync('output.txt', result);

// console.log(diplomas(2, 3, 10));