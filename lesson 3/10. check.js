...
function check(input) {
  const [count, ...dictionary] = input.trim().split('\n');
  let text = dictionary.pop();

  if(Number(count) > dictionary.length) {
    dictionary.push(text);
    text = '';
  }

  const map = dictionary.reduce((acc, currentValue) => {
    const lower = currentValue.toLowerCase();

    if(acc[lower]) {
      acc[lower].push(currentValue);
    } else {
      acc[lower] = [currentValue];
    }
  ...
  text.split(' ').forEach(word => {
    ...
    const replacedCapitalizeLetters = word.replace(/[A-Z]*/g, '');
    const countCapitalizedLetters = word.length - replacedCapitalizeLetters.length;

    if((map[lower] && !map[lower].includes(word)) || lower === word || countCapitalizedLetters >= 2) {
      errorCount++;
    }
  })
